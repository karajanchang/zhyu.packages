...
class Controller extends BaseController
{
	...
	protected function view($view = 'index', Model $model = null, $params = null){
		...
		$id = null;
		$addOrUpdateUrl = route($model->getTable().'.index');
		if(isset($model->id)){
			$id = $model->id;
			$addOrUpdateUrl = route($model->getTable().'.update', [ 'id' => $id]);
		}
		...
		return view()->first([$view, 'vendor.zhyu.form'], compact('table', 'title', 'id', 'datatablesService', $model_name, 'addOrUpdateUrl', 'id'));
	}
}